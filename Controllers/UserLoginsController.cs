...
namespace ZONAUTO.Controllers
{
    public class UserLoginsController : Controller
    {
        ...
        public async Task<IActionResult> Login(string email, string password, string tipoUsuario)
        {
            if (!ValidateLoginInput(email, password, tipoUsuario, out var errorResult))
                return errorResult;

            email = email.Trim().ToLower();

            var tipo = await _context.TiposUsuarios
                .AsNoTracking()
                .FirstOrDefaultAsync(t => t.Nombre == tipoUsuario);

            if (tipo == null)
                return InvalidUserTypeResult();

            var usuario = await GetUsuarioByCredentials(email, tipo.TipoUsuarioId);

            if (!ValidateUsuario(usuario, password, email, out var validationResult))
                return validationResult;

            await RegisterSuccessfulLogin(usuario);
            await SetUserSessionAndClaims(usuario, tipoUsuario);
            SetVerificationStatusMessage(usuario);

            return RedirectByUserType(tipoUsuario);
        }
        ...
        public async Task<IActionResult> ReenviarConfirmacion(string email)
        {
            ...
            await SendVerificationEmail(usuario.Persona.Email, usuario.Persona.Nombre, usuario.TokenVerificacion);
        ...
        public async Task<IActionResult> ConfirmarCuenta(string token)
        {
            ...
            await UpdateVerifiedUserSession(usuario);
        ...
        private bool ValidateLoginInput(string email, string password, string tipoUsuario, out IActionResult result)
        {
        ...
        private IActionResult InvalidUserTypeResult()
        {
            TempData["Error"] = "Tipo de usuario no válido.";
            ViewBag.TiposUsuario = GetTiposUsuarioSelectList();
            return View();
        }

        private async Task<Usuario> GetUsuarioByCredentials(string email, int tipoUsuarioId)
        {
            return await _context.Usuarios
                .Include(u => u.Persona)
                .Include(u => u.TipoUsuario)
                .FirstOrDefaultAsync(u =>
                    u.Persona.Email.ToLower() == email &&
                    u.TipoUsuarioId == tipoUsuarioId);
        }
        ...
        private async Task RegisterSuccessfulLogin(Usuario usuario)
        {
        ...
        private async Task SetUserSessionAndClaims(Usuario usuario, string tipoUsuario)
        {
        ...
        private async Task UpdateVerifiedUserSession(Usuario usuario)
        {
        ...
        private SelectList GetTiposUsuarioSelectList()
        {
            return new SelectList(_context.TiposUsuarios.AsNoTracking().ToList(), "Nombre", "Nombre");
        }
        ...
        private async Task SendVerificationEmail(string email, string nombre, string token)
        {
            try
            {
